...
XnVPointDrawer::XnVPointDrawer(XnUInt32 nHistory, xn::DepthGenerator depthGenerator) :
	...
	m_MoveCursorOnly(true),cursorX(0.0f),cursorY(0.0f),numOfMoves(0)
{
...
void XnVPointDrawer::OnPointUpdate(const XnVHandPointContext* cxt)
{
	...
	cursorX=ptProjective.X;
	cursorY=ptProjective.Y;
	...
	*/
	...
	if (!m_MoveCursorOnly){
		// Add new position to the history buffer
		m_History[numOfMoves].push_front(ptProjective);

		// Keep size of history buffer
		if (m_History[numOfMoves].size() > m_nHistorySize)
			m_History[numOfMoves].pop_back();
	}
...
void XnVPointDrawer::Draw() const
{
	...
	//set the pointer to be black
		glColor4f(0.0,0.0,0.0,1.0f);
		glRectf(cursorX-5,cursorY-5,cursorX+5,cursorY+5);
		glFlush();
}
...
void XnVPointDrawer::setMove(XnBool m){
	m_MoveCursorOnly=m;
	if (m)	numOfMoves++;
}