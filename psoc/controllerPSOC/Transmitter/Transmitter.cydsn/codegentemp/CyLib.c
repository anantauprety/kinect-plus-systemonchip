...
#include <CyLib.h>
    ...
    /* Variables must be placed beyond the first 256 bytes to ensure they do
     * not get clobbered by SROM boot code during the software reset.  They
     * are placed at the very end of SRAM to reduce the backward compatablity
     * impacts of reserving specific addresses.
     */
    uint8 CYXDATA CyResetStatus _at_ (CYDEV_SRAM_SIZE - 1);
    uint8 CYXDATA _CyResetInduced _at_ (CYDEV_SRAM_SIZE - 2); /* This should not be used outside of boot code */
#else
