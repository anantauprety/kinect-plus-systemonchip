...
uint8 CapSense_posFiltersData[] = {
    ...
    uint8 CapSense_position[CapSense_TOTAL_CENTROIDS_COUNT] = {0xFFu};
...
void CapSense_BaseInit(uint8 sensor)
{
    ...
    {
        CapSense_debounceCounter[widget - CapSense_TOTAL_CENTROIDS_COUNT] =  CapSense_debounce[widget];
    }
 ...
 void CapSense_UpdateSensorBaseline(uint8 sensor)
{
    ...
    #endif  /* End (CapSense_AUTO_RESET == CapSense_AUTO_RESET_DISABLE) */
...
uint8 CapSense_CheckIsSensorActive(uint8 sensor)
{
    ...
    if (widget < CapSense_TOTAL_CENTROIDS_COUNT)
    {
        ...
        CapSense_debounceCounter[debounceIndex] = 0u;
    }
    else
    {
        debounceIndex = widget - CapSense_TOTAL_CENTROIDS_COUNT;
    }
    ...
    else    /* Was off */
    {
        ...
        if (CapSense_SensorSignal[sensor] > (fingerThreshold + hysteresis))
        {
            ...
            if (CapSense_debounceCounter[debounceIndex]-- == 0)
            {
                CapSense_SensorOnMask[pos] |= onMask; 
            }
        }
...
uint8 CapSense_CalcCentroid(uint8 maximum, uint8 offset, uint8 count, uint16 resolution, uint8 noiseThreshold)
{
    ...
    position = ((uint8) HI16(denominator + 0x7F00u));
...
uint16 CapSense_GetCentroidPos(uint8 widget)
{
    ...
    {
        /* Calculate centroid */
        position = (uint16) CapSense_CalcCentroid(maximum, offset, count, CapSense_centroidMult[widget], CapSense_noiseThreshold[widget]);
        ...
        if ((posFiltersMask & CapSense_ANY_POS_FILTER) != 0u)
        {
            ...
            {
                /* Do filtering */
                if ((posFiltersMask & CapSense_IIR4_FILTER) != 0u)
                {
