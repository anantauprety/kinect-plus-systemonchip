...
#include "CapSense_TunerHelper.h"
    ...
    volatile CapSense_MAILBOXES CapSense_mailboxesComm;
        ...
        extern uint8 CapSense_position[CapSense_TOTAL_CENTROIDS_COUNT];
...
void CapSense_TunerComm(void)
{
    ...
    *******************************************************************************/
    void CapSense_ProcessAllWidgets(volatile CapSense_OUTBOX *outbox)
    {
        ...
        #if (CapSense_TOTAL_CENTROIDS_COUNT && (CapSense_TUNING_METHOD != CapSense_NO_TUNING))
        ...
        /* Calculate widget with centroids */
            ...
            {
                if(CapSense_GetTouchCentroidPos(i) == 0u)
                {
                ...
                {
                    outbox->position[i] = CapSense_SWAP_ENDIAN16( (uint16) CapSense_position[i]);
                    outbox->position[i+1] = CapSense_SWAP_ENDIAN16( (uint16) CapSense_position[i+1]);
                }
            }
        #endif  /* End (CapSense_TOTAL_TOUCH_PADS_COUNT) */
